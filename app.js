...
veryOldInventors = inventors.filter((eachInventor)=>{
    ...
    return(eachInventor.year<1600 && eachInventor.year>1499)
...
// Complete the exercise in the space below:
inventorNames= inventors.map((eachInventor)=>{
return { first: eachInventor.first, last: eachInventor.last }
...
sortedByBirthYear = inventors.sort((eachInventor)=>{
return (eachInventor)
})
...
numbers.sort(); 
console.log(numbers); // [1, 10, 25, 40, 5, 100]
...
inventorNamedAda = inventors.find((eachInventor)=>{
    return eachInventor.first==="Ada"
...
firstLast = people.map((eachPeople)=>{
    let names= eachPeople.split(",")
...
// Complete the exercise in the space below:
isAdultPresent = devs.some((eachPeople)=>{

    return (2024-eachPeople.year >= 18)
...
// Complete the exercise in the space below:
isEveryone19OrOlder= devs.every((eachPeople)=>{
return 2024-eachPeople.year >=19
...
// Complete the exercise in the space below:
commentById= comments.find((eachId)=>{
    return eachId.id ===823423
...
// Complete the exercise in the space below:
idx = comments.findIndex((eachId)=>{
return eachId.id ===123523
...
totalYearsLived= inventors.reduce(({year,passed})=>{
return passed-year
